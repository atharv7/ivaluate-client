  ...
  renderStudent = (student,teacherId) => {
    ...
    return (
      <div className="StudentBox"
        key={student.id}
        style={{backgroundColor:student.lastGrade || 'grey'}}
      >
        <br/>
        <img
          src={student.photo}
        ...
        >
          <input
            ...
            placeholder="Full Name" onChange={this.onChangeHandler}
        ...
        <form
          id={student.id+'_gradeform'}
          style={{display:this.state[student.id + '_grade'] || 'none'}}
          onSubmit={
            (e)=>
              {
                ...
                giveGrade(this.state[student.id+'_editgrade'] || student.lastGrade,
                  this.state[student.id+'_editremark'] || '',
                  student.id,teacherId|1)
              }
          }
        >
          <select
            name={student.id + "_editgrade"}
            placeholder="Select"
            onChange={this.onChangeHandler}
      ...
      <input type="button" value="ASK A QUESTION" 
       ...
       { 
        ...
        <div> 
          <div
            className="bar red"
            style={{width:(this.props.students.filter(x=>x.lastGrade==='red').length/students.length)*100+'%'}}>
          </div>
          <div
            className="bar yellow"
            style={{width:(this.props.students.filter(x=>x.lastGrade==='yellow').length/students.length)*100+'%'}}>
          </div>
          <div
            className="bar green"
            style={{width:(this.props.students.filter(x=>x.lastGrade==='green').length/students.length)*100+'%'}}>
          </div>
          <div
            className="bar grey"
            style={{width:(this.props.students.filter(x=>x.lastGrade==='Not Graded Yet!').length/students.length)*100+'%'}}>
          </div>
        </div>
      }
      </div> 
